...
#### 관계 객체 시스템 (Relationship Objects)

**NEW 2025-12-02**: 각 거래처/고객/스타트업별 전용 저장소

```
관계 객체 = 특정 대상(스타트업/고객/파트너)의 모든 정보

포함 항목:
- 대화 히스토리 (지난 N회 미팅 전사록)
- 파일 저장소 (피칭덱, 재무자료, 계약서)
- 구조화된 데이터 (MRR, CAC, Team Size 등)
- 타임라인 (첫 컨택, 다음 미팅)
- AI 자동 추출 데이터
```
...
**상세**: [docs/Onno - 관계 객체 시스템.md](docs/Onno%20-%20관계%20객체%20시스템%20(Relationship%20Objects).md)
